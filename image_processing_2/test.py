...
def test_cumulative_energy():
    pattern = lab.compute_energy(load_greyscale_image('test_results\pattern_grey.png'))
    result = lab.cumulative_energy_map(pattern)
    expected = load_greyscale_image('test_results\\pattern_cumulative.png')

    for row in range(pattern['height']):
        print(pattern['pixels'][row*pattern['width']:(row+1)*pattern['width']])
    print()
    for row in range(pattern['height']):
        print(result['pixels'][row*pattern['width']:(row+1)*pattern['width']])
    print()
    for row in range(pattern['height']):
        print(expected['pixels'][row*pattern['width']:(row+1)*pattern['width']])
    print()
